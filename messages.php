<?php
	...
	function clearAllMessages() {
		mysql_query("DELETE FROM {$_SESSION['DB_PREFIX']}messages WHERE to_member_id = " . getLoggedOnMemberID());
	}
...
?>
			...
			function clearMessages(id) {
				post("editform", "clearAllMessages", "submitframe", {});
			}
			
			function action(id, actionstr) {
				navigate(actionstr + "&messageid=" + id);
			}
			
			function actionFormatter(el, cval, opts) {
				if (el != null && el != "") {
					return "<img src='images/action.png' onclick='javascript: action(" + opts.uniqueid + ", \"" + el + "\")' />";
				}
...
<?php			
	...
	$crud->sql = "SELECT A.id, A.status, A.subject, A.message, A.from_member_id, A.to_member_id, A.createddate, A.action, " .
