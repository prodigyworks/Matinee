	...
	if (options.droppable) {
		$(document)
			.bind('dragstart', function(ev, ui) {
				var _e = ev.target;
				var e = $(_e);
				if (!e.parents('.fc1').length) { // not already inside a calendar
...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		for (i=0; d < maxd; i++) {
			...
			s +=
				...
				"</th>" +
				"<td class='fc-agenda-class " + contentClass + "'>" +
				"<div style='position:relative'>&nbsp;</div>" +
				"</td>" +
				"</tr>";
			minArray.push(formatDate(d, opt('axisFormat')));
		...
		var triggerCells = slotTable.find('.fc-agenda-class'); 
		
		triggerCells.each(function(i, _cell) {
			var t = minArray[i];
			trigger('timeRender', t, t, $(_cell));
		});
		...
		hoverListener.start(function(cell) {
			...
			if (cell) {
				if (cellIsAllDay(cell)) {
					...
					d1.setHours(d1.getHours() - 2); // KEVIN HILTON FIX
	...
	function dragStop(_dragElement, ev, ui) {
		...
		if (cell) {
			var d1 = cellDate(cell);
			d1.setHours(d1.getHours() - 2); // KEVIN HILTON FIX
			trigger('drop', _dragElement, d1, cellIsAllDay(cell), ev, ui);
		}
	}
